...
const QString BITCOIN_IPC_PREFIX("vericoin:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/vericoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/vericoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/vericoin-paymentrequest";
#endif
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // vericoin: URI
        {
...
#endif
{
    ...
    // on Mac: sent when you click vericoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start vericoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling vericoin: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("vericoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'vericoin://' is not a valid URI. Use 'vericoin:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // vericoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Vericoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in vericoin: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom vericoin addresses are not supported
