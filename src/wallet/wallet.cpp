...
CWallet::Balance CWallet::GetBalance(const int min_depth, bool avoid_reuse) const
{
    ...
    isminefilter reuse_filter = avoid_reuse ? ISMINE_NO : ISMINE_USED;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (tx_depth >= COINBASE_MATURITY+10) {
...
bool CWallet::SelectCoinsForStaking(interfaces::Chain::Lock& locked_chain, CAmount& nTargetValue, unsigned int nSpendTime, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    std::vector<COutput> vCoins;
    AvailableCoinsForStaking(locked_chain, vCoins);
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    ...
    if (coin_selection_params.use_bnb) {
        ...
        CAmount cost_of_change = GetDiscardRate(*this).GetFee(coin_selection_params.change_spend_size) + coin_selection_params.effective_fee.GetFee(coin_selection_params.change_output_size, true);
        ...
        for (OutputGroup& group : groups) {
            ...
            for (auto it = group.m_outputs.begin(); it != group.m_outputs.end(); ) {
                ...
                CAmount effective_value = coin.txout.nValue - (coin.m_input_bytes < 0 ? 0 : coin_selection_params.effective_fee.GetFee(coin.m_input_bytes, true));
                // Only include outputs that are positive effective value (i.e. not dust)
                if (effective_value > 0) {
                    group.fee += coin.m_input_bytes < 0 ? 0 : coin_selection_params.effective_fee.GetFee(coin.m_input_bytes, true);
                    group.long_term_fee += coin.m_input_bytes < 0 ? 0 : long_term_feerate.GetFee(coin.m_input_bytes, true);
        ...
        CAmount not_input_fees = coin_selection_params.effective_fee.GetFee(coin_selection_params.tx_noinputs_size, true);
        bnb_used = true;
...
uint64_t CWallet::GetStakeWeight(interfaces::Chain::Lock& locked_chain) const
{
    ...
    if (!SelectCoinsForStaking(locked_chain, nTargetValue, GetTime(), setCoins, nValueIn))
        return 0;
    ...
    {
        // XXX: Need to come back later with more data
        CTxIndex txindex;
        {
...
bool CWallet::CreateCoinStake(interfaces::Chain::Lock& locked_chain, const FillableSigningProvider& keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setCoins)
    {
                ...
                && pcoin.first->GetHash() != txNew.vin[0].prevout.hash)
        {
            ...
            if (nCredit >= GetStakeCombineThreshold() && nTimeWeight < STAKE_SPLIT_AGE)
                break;
            ...
            if (pcoin.first->tx->vout[pcoin.second].nValue >= GetStakeCombineThreshold() && nTimeWeight < STAKE_SPLIT_AGE)
                continue;

            // Do not add input that is still too young
            if (nTimeWeight < consensusParams.nStakeMinAge)
                continue;
    ...
    // Calculate coin age reward
    {
        ...
        // XXX: How to handle well switch to PoST
    ...
    // Get starting fee
    CAmount nMinFee = GetRequiredFee(*this, 0);
    while(true)
    {
        // Set output amount
        ...
        if (nMinFee < GetRequiredFee(*this, nBytes) - nMinFeeBase)
        {
            nMinFee = GetRequiredFee(*this, nBytes) - nMinFeeBase;
...
int CWalletTx::GetBlocksToMaturity(interfaces::Chain::Lock& locked_chain) const
{
    ...
    return std::max(0, (COINBASE_MATURITY+10) - chain_depth);
}
